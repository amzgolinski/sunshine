...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
   ...
   *
   * @param locationSetting The location string used to request updates from
   *                        the server.
   ...
   */
  private long addLocation(String locationSetting, String cityName, double lat,
                           double lon) {
    long locationId;
    ...
    if (locationCursor.moveToFirst()) {
      ...
      // Now that the content provider is set up, inserting rows of data is
      // pretty simple. First create a ContentValues object to hold the data
      // you want to insert.
  ...
  private void getWeatherDataFromJson(String forecastJsonStr,
    ...
    try {
      ...
      // OWM returns daily forecasts based upon the local time of the city that
      // is being asked for, which means that we need to know the GMT offset to
      // translate this data properly.
      ...
      for (int i = 0; i < weatherArray.length(); i++) {
        ...
        // long. That element also contains a weather code.
    ...
    if (displayNotifications) {
      ...
      if (System.currentTimeMillis() - lastSync >= DAY_IN_MILLIS) {
        ...
        if (cursor.moveToFirst()) {
          ...
          // On Honeycomb and higher devices, we can retrieve the size of the
          // large icon Prior to that, we use a fixed size
  ...
  private static void onAccountCreated(Account newAccount, Context context) {
    ...
    SunshineSyncAdapter.configurePeriodicSync(
        context, SYNC_INTERVAL, SYNC_FLEXTIME);
