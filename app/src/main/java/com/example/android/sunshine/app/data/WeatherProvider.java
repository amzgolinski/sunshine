...
public class WeatherProvider extends ContentProvider {
  ...
  static UriMatcher buildUriMatcher() {
    ...
    // regular expressions instead?  Because you're not crazy, that's why.

    // All paths added to the UriMatcher have a corresponding code to return
    // when a match is found.  The code passed into the constructor represents
    // the code to return for the root
  ...
  public Cursor query(Uri uri, String[] projection, String selection,
    ...
    // Here's the switch statement that, given a URI, will determine what kind
    // of request it is, and query the database accordingly.
    Cursor retCursor;
    switch (sUriMatcher.match(uri)) {
      // "weather/*/*"
      case WEATHER_WITH_LOCATION_AND_DATE: {
        retCursor =
            getWeatherByLocationSettingAndDate(uri, projection, sortOrder);
        break;
      }
      // "weather/*"
      case WEATHER_WITH_LOCATION: {
  ...
  public Uri insert(Uri uri, ContentValues values) {
    ...
    switch (match) {
      ...
      case LOCATION: {
        long _id =
            db.insert(WeatherContract.LocationEntry.TABLE_NAME, null, values);
        if (_id > 0)
  ...
  // assist the testing framework in running smoothly. You can read more at:
