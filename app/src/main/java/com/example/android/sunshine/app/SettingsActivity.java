...
public class SettingsActivity extends PreferenceActivity
  ...
  // Registers a shared preference change listener that gets notified when
  // preferences change
  @Override
  ...
  protected void onActivityResult(int requestCode, int resultCode,
    ...
    if (requestCode == PLACE_PICKER_REQUEST) {
      ...
      if (resultCode == RESULT_OK) {
        ...
        // TODO(student): Get the LatLng object from the place -- this is a
        // placeholder.
        LatLng latLong = place.getLatLng();

        // If the provided place doesn't have an address, we'll form a
        // display-friendly string from the latlng values.
        ...
        // Also store the latitude and longitude so that we can use these to
        // get a precise result from our weather service. We cannot expect the
        // weather service to understand addresses that Google formats.
        // TODO(student) Store the latitude and longitude as float values
        // according to the keys defined in strings.xml.
        ...
        // Tell the SyncAdapter that we've changed the location, so that we can
        // update our UI with new values. We need to do this manually because
        // we are responding to the PlacePicker widget result here instead of
        // allowing the LocationEditTextPreference to handle these changes and
        ...
        if (mAttribution != null) {
          ...
          // For pre-Honeycomb devices, we cannot add a footer, so we will use
          // a snackbar
  ...
  public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
    ...
    if (key.equals(getString(R.string.pref_location_key))) {
      ...
      // Wipe out any potential PlacePicker latlng values so that we can use
      // this text entry.
